...
class LLMClassifier(ComplaintClassifier):
    ...
    def _call(self, text: str, meta: dict) -> dict:
        """
    ...
    def categorize(self, text: str, meta: Optional[dict] = None) -> CategoryResult:
        ...
        """
        p = self._call(text, meta or {})
        return CategoryResult(category=p["category"], sub_category=p.get("sub_category"))

    def analyze_sentiment(self, text: str, meta: dict | None = None) -> SentimentResult:
        """
        Perform sentiment mapping, urgency flags, and environmental health risk analysis.
        """
        p = self._call(text, meta or {})
        return SentimentResult(label=p["sentiment_label"], is_urgent=p["is_urgent"], health_risk_detected=p["health_risk_detected"])
